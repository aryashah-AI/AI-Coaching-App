...
class AthleteService:
    ...
    def calculate_joint_angles(self, trajectories):
        ...
        for i in range(len(trajectories["left_ankle"])):
            if (
                    trajectories["left_ankle"][i] is not None
                    and trajectories["left_knee"][i] is not None
                    and trajectories["left_hip"][i] is not None
            ):
            ...
            if (
                    trajectories["right_ankle"][i] is not None
                    and trajectories["right_knee"][i] is not None
                    and trajectories["right_hip"][i] is not None
            ):
    ...
    def detect_events(
            self, foot_left_traj, foot_right_traj, min_height_threshold=0.1,
            min_distance=10):
        """
        ...
        for i in range(1, len(velocity_smooth) - 1):
            ...
            if (
                    velocity_smooth[i - 1] > 0
                    and velocity_smooth[i] < -min_height_threshold
            ):
            ...
            # Landing: transition from negative to positive velocity (going down)
                    ...
                    velocity_smooth[i - 1] < 0 and velocity_smooth[
    ...
    def classify_phases(self, events):
        ...
        for i in range(0, len(events) - 1, 2):
            if (
                    i + 1 < len(events)
                    and events[i] == "takeoff"
                    and events[i + 1] == "landing"
            ):
    ...
    def create_annotated_video(self, video_path, results, output_video_path):
        ...
        while True:
            ...
            if frame_idx in event_frames:
                ...
                event_color = (
                    (0, 255, 255) if event_type == "takeoff" else (255, 255, 0)
                )
                cv2.putText(
                    frame,
    ...
    def process_video(self, video_path, output_dir=None,
        ...
        events = self.safe_detect_events(
            trajectories["left_ankle"], trajectories["right_ankle"]
        )
        ...
        if output_dir:
            ...
            viz_path = os.path.join(output_dir, "analysis_results.png")
            ...
            if create_video_output:
                ...
                logger.info("Creating annotated video output...")
                video_name = os.path.splitext(os.path.basename(video_path))[0]
                output_video_path = os.path.join(
                    output_dir, f"{video_name}_annotated.webm"
                )
                logger.info(
                    f"Output video will be saved to: {output_video_path}")
                try:
                    self.create_annotated_video(video_path, results,
                                                output_video_path)
                    if os.path.exists(output_video_path):
                        logger.info(
                            f"Annotated video created successfully: {output_video_path}")
                    else:
                        logger.error(
                            f"Annotated video was not created: {output_video_path}")
                except Exception as e:
    ...
    def process_training_videos(self, video_paths: List[str]) -> Dict[
        ...
        for video_path in video_paths:
            try:
                ...
                logger.info(f"Processing training video: {video_path}")

                # Process video using existing detector
                results, _ = self.process_video(video_path,
        ...
        logger.info(
            f"Successfully processed {successful_videos} training videos")
        return phase_data
    ...
    def _extract_phase_angles(self, results: Dict,
        ...
        for phase_idx, phase in enumerate(phases):
            ...
            end_frame = min((phase_idx + 1) * frames_per_phase,
                            frame_count)
    ...
    def train_model_from_videos(self, video_paths: List[str],
                                model_name: str = "triple_jump_model.pkl") -> \
            Optional[str]:
        """
        ...
        Args:
            video_paths: List of paths to training videos
            model_name: Name of the model file to create

        Returns:
            Path to the saved model if successful, None otherwise
        """
        logger.info("=== TRIPLE JUMP MODEL TRAINING ===")
        ...
        if len(existing_videos) < 2:
            logger.error(
                f"Need at least 2 training videos. Found: {len(existing_videos)}")
            logger.info(f"Available videos: {existing_videos}")
            return None

        logger.info(
            f"Training with {len(existing_videos)} videos: {existing_videos}")

        # Initialize trainer and create model
        model_path = os.path.join("base/static/uploads", model_name)

        try:
            model = self.create_model(existing_videos, model_path)
            logger.info(f"✓ Model training completed successfully!")
            logger.info(f"✓ Model saved to: {model_path}")
            logger.info(f"✓ Training data: {len(existing_videos)} videos")
            return model_path
...
class TripleJumpAnalyzer:
    ...
    def analyze_performance(self, video_path: str,
        ...
        """
        logger.info(f"Starting performance analysis for video: {video_path}")

        # Process the input video
        results, output_video_path = self.detector.process_video(video_path,
                                                                 output_dir,
                                                                 create_video_output=True)
        ...
        logger.info(
            f"Performance analysis completed. Generated {len(performance_metrics)} phase metrics.")
        return results, performance_metrics, output_video_path
